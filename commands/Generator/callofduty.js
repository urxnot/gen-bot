...
module.exports.run = async (client, message, args) => {
  ...
  fs.readFile('./accounts/callofduty.txt', (err, file) => {
    if (file.length == 0) {
      ...
      fs.readFile('./accounts/callofduty.txt', 'utf8', function (err, data) {
        if (err) throw err;
        ...
        fs.writeFile('./accounts/callofduty.txt', lines.slice(1).join('\n'), function (err) {
          if (err) throw err;
        });
...
module.exports.help = {
  ...
  category: 'generator',
  description: 'generate a call of duty account',
  cooldown: 900,
  usage: '',
  example: [],
};
