...
module.exports.run = async (client, message, args) => {
  ...
  fs.readFile('./accounts/valorant.txt', (err, file) => {
    if (file.length == 0) {
      ...
      fs.readFile('./accounts/valorant.txt', 'utf8', function (err, data) {
        if (err) throw err;
        ...
        fs.writeFile('./accounts/valorant.txt', lines.slice(1).join('\n'), function (err) {
          if (err) throw err;
        });
...
module.exports.help = {
  ...
  aliases: ['valorant'],
  category: 'generator',
  description: 'generate a valorant account',
  cooldown: 900,
  usage: '',
  example: [],
};
