...
module.exports.run = async (client, message, args) => {
  ...
  fs.readFile('./accounts/pornhub.txt', (err, file) => {
    if (file.length == 0) {
      ...
      fs.readFile('./accounts/pornhub.txt', 'utf8', function (err, data) {
        if (err) throw err;
        ...
        fs.writeFile('./accounts/pornhub.txt', lines.slice(1).join('\n'), function (err) {
          if (err) throw err;
        });
        const acc = account.split(':');
        message.author.send({
          embed: {
            title: 'Pornhub Account',
...
module.exports.help = {
  name: 'pornhub',
  aliases: ['pornhub', 'ph', 'php'],
  category: 'generator',
  description: 'generate a pornhub account',
  cooldown: 900,
  usage: '',
  example: [],
};
